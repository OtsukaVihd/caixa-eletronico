...
class BankAccountFileReader(BankFile):
    ...
    def get_account(self, account_number):
        lines = self._readlines()
        lines = self.__skip_first_line(lines) #ele nao alterou aq
        bank_account = None
        for line in lines:
            bank_account_created = self.__create_bank_account_from_file_line(line)
            if bank_account_created.check_account_number(account_number):
...
class BankAccountFileWriter(BankFile):
    ...
    def write_bank_account(self, bank_account):
        ...
        lines = self._readlines()
        lines[line_index_to_update] = self.format_line_to_write(bank_account)
        self._writelines(bank_account)

    def format_line_to_write(self, bank_account):
        line = '%s;%s;%s;%s;%s;' % (
...
class MoneySlipsFileReader(MoneySlipsFile ,BankFile):
...
class MoneySlipsFileWriter(MoneySlipsFile ,BankFile):
